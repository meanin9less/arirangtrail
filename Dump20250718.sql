...
CREATE TABLE `chatrooms` (
  `roomid` bigint NOT NULL AUTO_INCREMENT COMMENT '채팅방 고유 ID, 이 방의 모든 채팅 메시지들이 이 ID를 참조함',
  `title` varchar(255) NOT NULL COMMENT '채팅방 제목',
  `creatorusername` varchar(20) NOT NULL COMMENT '채팅방 개설자 (users 테이블의 username 참조)',
  `lastmessageseq` bigint NOT NULL DEFAULT '0' COMMENT '이 채팅방의 마지막 메시지 순번. 새 메시지가 올 때마다 1씩 증가시킴',
  ...
  PRIMARY KEY (`roomid`),
  KEY `fk_rooms_to_users` (`creatorusername`),
  CONSTRAINT `fk_rooms_to_users` FOREIGN KEY (`creatorusername`) REFERENCES `users` (`username`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='채팅방 정보 마스터 테이블';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='리뷰 사진 정보';
/*!40101 SET character_set_client = @saved_cs_client */;
...
-- Dump completed on 2025-07-18 10:13:42
